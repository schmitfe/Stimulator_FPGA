...
architecture Behavioral of Channel is
	...
	signal counter : integer range 0 to MaxDelay+SamplingTime := 0;
...
begin
	...
	Counter : process (CLK) --counter for delay in state transition
	begin
		if CLK = '1' and CLK'EVENT then
...
-------------------Transmit---------------------------------------				
			...
			when CounterR_II => FolState <= TStartTX_WF2 after 5 ns;	
			...
			when TRead_WF2 => FolState <= TRead_WF2 after 5ns;
				...
				if TX_Done = '1' and  Nullflag='1' and counter>=SamplingTime+InterPeriod then
					FolState <= TWAIT_WF1 after 5 ns;
				end if;
